package com.telran.oscar.data;

public class UserData {
    public static final String USER_LOGIN_NAME = "MaxMiller";
    public static final String USER_LOGIN_EMAIL = "miller@gm.com";
    public static final String USER_LOGIN_PASSWORD = "Miller999!";


    public static final String FIRST_NAME = "Tata";
    public static final String LAST_NAME = "Korn";
    public static final String ADDRESS = "Schnelstrasse";
    public static final String CITY = "Berlin";
    public static final String POSTCODE = "12439";

//data for deleting user
   public static final String USER_REG_PASSWORD = "BPRno1456$";



}
